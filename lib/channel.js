...
var PermissionSet = require("./channel/permissions");
var Banlist = require("./channel/banlist");
...
function Channel(name) {
    ...
    self.poll = null;
    self.voteskip = null;
    self.permissionSet = new PermissionSet();
    self.banlist = new Banlist(self);
    self.permissions = {
...
Channel.prototype.getIPRank = function (ip, callback) {
    ...
    db.getAliases(ip, function (err, names) {
        ...
        db.users.getGlobalRanks(names, function (err, res) {
            ...
            var globalRank = res.reduce(function (a, b) {
                return Math.max(a, b);
            }, 0);

            if (!self.registered) {
                callback(null, globalRank);
                return;
            }
...
Channel.prototype.join = function (user) {
    ...
    var afterLogin = function () {
        ...
        Account.getAccount(user.name, user.ip, { channel: self.name }, function (err, acc) {
            user.account = acc;
        });
        ...
        self.logger.log("[login] " + user.ip + " logged in as " + user.name);
    };
...
Channel.prototype.handleNameBan = function (actor, name, reason) {
    ...
    return self.banlist.banName(name, actor, reason);
