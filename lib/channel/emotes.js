...
EmoteModule.prototype.postJoin = function (user) {
...
EmoteModule.prototype.handleImportEmotes = function (user, data) {
    ...
    /* Note: importing requires a different permission node than simply
       updating/removing */
    if (!this.channel.modules.permissions.canImportEmotes(u)) {
        return;
    }
...
EmoteModule.prototype.handleRemoveEmote = function (user, data) {
    ...
    if (!this.channel.modules.permissions.canEditEmotes(u)) {
        return;
    }
    ...
    this.channel.logger.log("[mod] " + user.getName() + " removed emote: " + f.name);
};
