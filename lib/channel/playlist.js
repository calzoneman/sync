...
PlaylistModule.prototype.handleMoveMedia = function (user, data) {
    ...
    self.semaphore.queue(function (lock) {
        if (!self.items.remove(data.from)) {
            console.log('DBG: Failed to remove ' + data.from);
            return lock.release();
        }

        if (data.after === "prepend") {
            if (!self.items.prepend(from)) {
                console.log('DBG: Failed to prepend ' + data.from);
                return lock.release();
            }
        } else if (data.after === "append") {
            if (!self.items.append(from)) {
                console.log('DBG: Failed to append ' + data.from);
                return lock.release();
            }
        } else {
            if (!self.items.insertAfter(from, data.after)) {
                console.log('DBG: Failed to insertAfter ' + data.from + ', ' + data.after);
                return lock.release();
            }
        }

        self.channel.users.forEach(function (u) {
            u.socket.emit("moveVideo", data);
        });
...
PlaylistModule.prototype.handleAssignLeader = function (user, data) {
    ...
    for (var i = 0; i < this.channel.users.length; i++) {
        if (this.channel.users[i].getName() === name) {
            ...
            var self = this;
            this.channel.users.forEach(function (u) {
                u.socket.emit("setLeader", name);
                if (self.leader.account.effectiveRank === 1.5) {
                    u.socket.emit("setUserRank", {
                        name: name,
                        rank: 1.5
                    });
