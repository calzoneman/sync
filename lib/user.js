...
User.prototype.setAFK = function (afk) {
    ...
    this.channel.checkVoteskipPass();
    this.channel.sendAll("setAFK", {
        name: this.getName(),
        afk: afk
    });
};
...
User.prototype.login = function (name, pw) {
    ...
    db.users.verifyLogin(name, pw, function (err, user) {
        ...
        self.refreshAccount(opts, function (err, account) {
            ...
            if (self.inChannel()) {
                self.channel.logger.log(self.ip + " logged in as " + user.name);
            }
...
User.prototype.refreshAccount = function (opts, cb) {
    ...
    Account.getAccount(name, this.ip, opts, function (err, account) {
        if (!err) {
