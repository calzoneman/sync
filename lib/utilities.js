(function () {
    ...
    root.maskIP = function (ip) {
        ...
        } else if (ip.match(/^(?:[0-9a-f]{0,4}:){7}[0-9a-f]{0,4}/)) {
            return ip.replace(/^(?:[0-9a-f]{4}:){3}[0-9a-f]{4}/, "x:x:x:x");
        } else if (ip.match(/^(^|\s)(?:[0-9a-f]{4}:){3}[0-9a-f]{4}$/)) {
            return ip.replace(/^(?:[0-9a-f]{4}:){2}([0-9a-f]{4}):[0-9a-f]{4}$/, "x:x:$1:*");
        } else {
            return ip;
        }
    },
    ...
    root.getIPRange = function (ip) {
        if (net.isIPv6(ip)) {
            ...
            return ip.replace(/((?:[0-9]{3}\.){2}[0-9]{3})\.[0-9]{3}/, "$1");
        }
    },
